...
add_online_user(JID, Nick, Role, Packet, StateData) ->
    ...
    reset_hibernate_timer(update_online_user(JID, User, Packet, StateData)).
...
send_kickban_presence(UJID, JID, Reason, Code, NewAffiliation,
    ...
    lists:foreach(fun (LJ) ->
			  ...
			  add_to_log(kickban, {Nick, Reason, Code, LJ, NewAffiliation, UJID, get_actor_nick(UJID, StateData)}, StateData),
